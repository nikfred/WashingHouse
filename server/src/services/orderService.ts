import {IOrder, IOrderItem, IService} from "../types/types";

const Order = require('../models/order')
const OrderItem = require('../models/orderItem')
const ApiError = require('../error/ApiError')
const washhouseService = require('./washhouseService')
...
const checkUid = (uid: string, order: IOrder) => {
...
const checkNotActiveOrder = (order: any) => {
...
const checkActiveOrder = (order: any) => {
    if (order.status === allStatuses.complete[0] || order.status === allStatuses.cancel[0]) {
        throw ApiError.forbidden("Order is not active")
    }
}
...
class OrderService {
    async create(uid: string, washhouse_id: string, service_ids: Array<string>) {
        const user = await userService.getUser(uid)
        let {washhouse, services} = await washhouseService.getOne(washhouse_id)
        let total = 0 //стоимость всего заказа
        for (const service_id of service_ids) {
            total += +services.filter((i: IService) => (i._id).indexOf(service_id) !== -1)[0].price
        ...
        services = services.filter((i: IService) => service_ids.indexOf((i._id).toString()) !== -1)
        return {order: new OrderDto({...washhouse._doc, ...order._doc}), services}
    }

    async getAllUserOrders(uid: string) {
    ...
    async getOneDetail(id: string, uid: string = '') {
        const order = await this.getOne(id, uid)
        let {services} = await washhouseService.getOne(order.washhouse_id)
        const service_ids = (await OrderItem.find({order_id: id})).map((i: IOrderItem) => (i.service_id).toString())

        services = services.filter((i: IService) => service_ids.indexOf((i._id).toString()) !== -1)
        return {order, services}
    }

    async adminGetOneDetail(id: string) {
    ...
    async updateStatus(id: string, status: string, uid: string = '', reason: string = '') {
        const order = await Order.findById(id)
    ...
    async removeServices(id: string, remove: Array<string> = []) {
        let {order, services} = await this.getOneDetail(id)
        checkNotActiveOrder(order)
        //удаление повторяющихся услуг
        remove = [...(new Set(remove))]
        //подсчет возвращаемой суммы
        let refund = services.reduce((sum: number, i: IService) => {
        ...
        //объект для обновления информации о заказе
        let updateData: any = {total: (+order.total - refund)}
