...
class UserService {
    async registration(userData: any, password: string) {
    ...
    async login(email: string, password: string) {
        ...
        const equivalence = bcrypt.compareSync(password, user.password)
        if (!equivalence) {
    ...
    async getAllClient() {
        return (await User.find({role: "USER"})).map((i: IUser) => new UserProfileDto(i))
    }

    async update(id: string, rawUser: any) {
    ...
    async updateBalance(id: string, balance = 0) {
